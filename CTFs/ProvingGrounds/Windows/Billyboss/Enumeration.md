...
FTP isn't accessible anonymously.

```
ftp 192.168.158.61 21    
Connected to 192.168.158.61.
220 Microsoft FTP Service
Name (192.168.158.61:saitama): anonymous
...
- Endpoint Fuzzing --> gobuster (dirbuster wordlist) [x]
- dirsearch [x]
- gobuster file endpoints [x]
- feroxbuster [x]
- subdomain enum --> ffuf [x]
- nikto [x]
- robots.txt [x]
- Website analyzing [x]

There seems to be running an application called "BaGet".

Found API Index in http://192.168.158.61/v3/index.json & Version Information BaGet 3.0.0
## SMB 445

- smbclient -L

Couldn't enumerate shares anonymously.

```
smbclient -L \\\\192.168.158.61
Password for [WORKGROUP\root]:
session setup failed: NT_STATUS_ACCESS_DENIED
```

Couldn't enumerate shares with guest account.

```
smbclient -L \\\\192.168.158.61\\ -U 'guest'
